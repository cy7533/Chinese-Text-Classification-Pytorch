...
VocabTuple = namedtuple('VocabTuple', ['crime', 'judge', 'performance', 'fact', 'article'])
...
def build_dataset(config, use_word):
    ...
    if os.path.exists(config.vocab_path):
        ...
        print('Building article_vocab...')
        article_vocab = build_vocab(train_df.article.values, word_tokenizer, MAX_VOCAB_SIZE, 1, config.material_path)
        print('article_vocab', article_vocab)
        vocab = VocabTuple(crime=crime_vocab, judge=judge_vocab,
                           performance=performance_vocab, fact=fact_vocab, article=article_vocab)
        pkl.dump(vocab, open(config.vocab_path, 'wb'))
    print(f"Vocab size: crime: {len(vocab.crime)}, judge: {len(vocab.judge)}, performance: {len(vocab.performance)}, "
          f"fact: {len(vocab.fact)}, article: {len(vocab.article)}")
    ...
    if config.crime_n_vocab == 0:
        ...
        config.article_n_vocab = len(vocab.article)
    ...
    if config.crime_max_length == 0:
        ...
        config.article_max_length = get_max_lengths(train_df.article.values, vocab.article)
        ...
        print('article_max_length', config.article_max_length)
    ...
    def load_dataset(path):
        ...
        for i, row in tqdm(df.iterrows()):
            ...
            article_encode = encode_document(row['article'], vocab.article, config.article_max_length)
            contents.append({
                'crime': crime_encode,
                'judge': judge_encode,
                'performance': performance_encode,
                'fact': fact_encode,
                'article': article_encode,
                'label': row['label']
            })
        return contents
...
class DatasetIterater(object):
    ...
    def _to_tensor(self, data):
        ...
        x_article = torch.LongTensor([_['article'] for _ in data]).to(self.device)
