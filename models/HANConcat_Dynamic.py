...
from models.HAN.hierarchical_att_dynamic_model import HierAttDynamicNet
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.performance_encoder = HierAttNet(config.performance_word_hidden_size, config.performance_sent_hidden_size,
                                              ...
                                              config.device)
        self.fact_encoder = HierAttDynamicNet(config.fact_word_hidden_size, config.fact_sent_hidden_size,
                                              config.batch_size, config.fact_n_vocab, config.embed,
                                              config.crime_hidden_size, config.device)

        self.fc = nn.Linear(2 * (config.crime_hidden_size +
                                 config.judge_sent_hidden_size +
                                 config.performance_sent_hidden_size +
                                 config.fact_sent_hidden_size), config.num_classes)
    ...
    def forward(self, x):
        ...
        fact_output = self.fact_encoder(fact_input, crime_output)

        # output: [batch, 2 * (self.crime_hidden_size + self.judge_sent_hidden_size +
        # self.performance_sent_hidden_size)]
        output = torch.cat((crime_output, judge_output, performance_output, fact_output), dim=1)
